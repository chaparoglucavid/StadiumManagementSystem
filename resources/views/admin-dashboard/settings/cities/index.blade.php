                            ...
                            @foreach($cities as $key => $city)
                                <tr>
                                    ...
                                    <td class="d-flex align-items-center gap-2">
                                        <span class="title-text mb-0">{{ $city->getTranslation('city_name', app()->getLocale()) }}</span>
                                    </td>
                                    <td>
                                        <span class="title-text mb-0">{{ \Carbon\Carbon::parse($city->created_at)->format('d.m.Y') }}</span>
                                    </td>
                                    <td>
                                        <span
                                            class="badge text-light-{{ $city->status === "active" ? 'success' : 'danger' }}">{{ $city->status === "active" ? 'Aktiv' : 'Deaktiv' }}</span>
                                    </td>
                                    <td>
                                        <a class="btn btn-light-primary icon-btn w-30 h-30 b-r-22 me-2"
                                           href="{{ route('admin.cities.edit', encrypt($city->uid)) }}"
                                           role="button" target="_blank"> <i class="ti ti-eye"></i></a>
                                        <a href="{{ route('admin.cities.edit', encrypt($city->uid)) }}">
